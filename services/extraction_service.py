...
class PyMuPdfExtractor(PdfExtractor):
    ...
    def _extract_titleblock_region_text(self, doc: fitz.Document, page_num: int) -> str:
        """
        Extract text from the title block region of a page.
        The title block is typically in the bottom-right corner.
        ...
        Returns:
            Extracted text from the title block region
        """
        try:
            ...
            # Define title block region (bottom-right 40% width, bottom 30% height)
            titleblock_rect = fitz.Rect(
                page_rect.width * 0.6,  # Start at 60% of page width
                page_rect.height * 0.7,  # Start at 70% of page height
                page_rect.width,  # End at page width
                page_rect.height,  # End at page height
            )

            # Extract text from the region
            text = page.get_text("text", clip=titleblock_rect)
            return text.strip()

        except Exception as e:
            self.logger.error(f"Error extracting title block text from {os.path.basename(doc.name or 'unknown_file')}, page {page_num + 1}: {e}")
            return ""
