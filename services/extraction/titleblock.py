"""
Title block extraction and analysis utilities.

Pure functions for detecting, scoring, and extracting title block content from PDF pages.
"""
import re
import logging
...
def extract_titleblock_region_text(
    doc: fitz.Document, page_num: int, logger: Optional[logging.Logger] = None
) -> str:
    """
    ...
    Returns:
        Extracted title block text
    """
    if logger is None:
        logger = logging.getLogger(__name__)

    if page_num < 0 or page_num >= len(doc):
        logger.warning(f"Page number {page_num} out of range (0-{len(doc)-1})")
        return ""
    ...
    for x0_pct, y0_pct, x1_pct, y1_pct, region_name in regions:
        ...
        for expansion in expansions:
            ...
            try:
                ...
                logger.warning(f"Error extracting from region {region_name}: {e}")
                continue
            ...
            score = score_titleblock_text(text)
            ...
            if score >= 0.8 and not looks_truncated(text):
                logger.info(
            ...
            if looks_truncated(text) and expansion < 0.20:
                continue
    ...
    if best_text:
        logger.info(
            ...
            f"truncated: {looks_truncated(best_text)}"
        )
    else:
        logger.warning(f"No title block found on page {page_num}")

    return best_text


def score_titleblock_text(text: str) -> float:
    """
...
def extract_project_name_from_titleblock(
    titleblock_text: str,
    ...
    Args:
        titleblock_text: Title block text to analyze

    Returns:
        Tuple of (project_name, source, is_truncated)
    """
    ...
    patterns = [
        ...
        (r"JOB\s*(?:NAME)?\s*:?\s*([^\n\r]+)", "job_label"),
        (
            r"(?:^|\n)([A-Z][A-Z\s\-&]+(?:PROJECT|BUILDING|CENTER|FACILITY|COMPLEX|TOWER|PLAZA))",
            "inferred",
        ),
    ]
