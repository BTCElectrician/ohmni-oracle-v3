...
# Structured output storage configuration (defaults to original backend/connection settings)
STRUCTURED_STORAGE_BACKEND = os.getenv(
    "STRUCTURED_STORAGE_BACKEND",
    ORIGINAL_STORAGE_BACKEND,
).lower()
STRUCTURED_STORAGE_PREFIX = os.getenv("STRUCTURED_STORAGE_PREFIX", "structured-output").strip("/")
STRUCTURED_BLOB_CONNECTION_STRING = os.getenv(
    "STRUCTURED_BLOB_CONNECTION_STRING",
    AZURE_BLOB_CONNECTION_STRING,
)
STRUCTURED_BLOB_ACCOUNT_URL = os.getenv("STRUCTURED_BLOB_ACCOUNT_URL", AZURE_BLOB_ACCOUNT_URL)
STRUCTURED_BLOB_CREDENTIAL = os.getenv("STRUCTURED_BLOB_CREDENTIAL", AZURE_BLOB_CREDENTIAL)
...
def get_all_settings() -> Dict[str, Any]:
    return {
        ...
        "STRUCTURED_STORAGE_BACKEND": STRUCTURED_STORAGE_BACKEND,
        "STRUCTURED_STORAGE_PREFIX": STRUCTURED_STORAGE_PREFIX,
        "STRUCTURED_BLOB_CONNECTION_STRING": "***REDACTED***"
        if STRUCTURED_BLOB_CONNECTION_STRING
        else None,
        "STRUCTURED_BLOB_ACCOUNT_URL": STRUCTURED_BLOB_ACCOUNT_URL,
        "STRUCTURED_BLOB_CREDENTIAL": "***REDACTED***" if STRUCTURED_BLOB_CREDENTIAL else None,
        "STRUCTURED_BLOB_SAS_TOKEN": "***REDACTED***" if STRUCTURED_BLOB_SAS_TOKEN else None,
        "STRUCTURED_BLOB_CONTAINER": STRUCTURED_BLOB_CONTAINER,
    }
